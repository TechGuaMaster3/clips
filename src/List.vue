      ...
      <v-container class="mb-4">
        <v-tabs icons-and-text centered dark background-color="teal">
          ...
          <v-tab-item value="tab-1">
            <v-card class="pa-2" color="teal lighten-5" flat>
              ...
              <div>[表單連結] <a target="_blank" href="http://twitch.center/customapi/quote/list?token=96a4694e" style="cursor: pointer;">
              http://twitch.center/customapi/quote/list?token=96a4694e</a> </div>
            </v-card>
          </v-tab-item>
        ...
        <v-layout wrap class="cliparea">
          ...
          <template v-if="!loading">
            <v-flex xs12 sm6 md4 lg3 xl2 v-for="(n,index) in Math.min(startcount+1, page_count)"
              :key="index">
              <v-card color="teal lighten-5" class="ma-1 clipcard">
                <a style="position:relative"
                  :href="clipList[clipList.length-(page-1)*page_count-n].url" target="_blank">
                  <v-img class="cardimage"
                    :src="clipList[clipList.length-(page-1)*page_count-n].thumbnail_url"
                    height="200px">
                  </v-img>
                </a>
                <v-card-text class="wb">
                  <a :href="clipList[clipList.length-(page-1)*page_count-n]['url']" target="_blank">
                    <v-flex xs12>
                      {{`[${clipList[clipList.length-(page-1)*page_count-n]['Num']}.] `}}
                      {{clipList[clipList.length-(page-1)*page_count-n]['title']}}
                    </v-flex>
                  </a>
                  <v-flex xs12 class="mt-2" style="font-size:1.1rem"
                    v-if="clipList[clipList.length-(page-1)*page_count-n]['creator_id']!=''">
                    剪輯：
...
export default {
  ...
  methods: {
    async getRequest(url) {
      const Jdata = await (await (fetch(url)
        .then((res) => res.json())
        .catch((err) => {
    ...
    async getTwitchRequest(url) {
      ...
      const Jdata = await (await (fetch(`${API_BASE}${url}`)
        .then((res) => {
          if (res.ok) {
            this.loadtext = '取得資料中...';
            return res.json();
    ...
    async getList(val) {
      /** Clips Page Range */
      const startcount = this.clipJSON.length - (val - 1) * this.page_count - 1;
      ...
      /** Get Clips Data */
      const ClipsBaseURL = 'clips?';
      let TempURL = ClipsBaseURL;
      for (let i = startcount; i > endcount; i -= 1) {
        TempURL += `&id=${this.clipJSON[i].ID}`;
      }
      const ClipsResponse = await this.getTwitchRequest(TempURL);
      if (ClipsResponse) {
        ClipsResponse.data.forEach((element) => {
          element.Num = this.clipNum[element.id].Num; // eslint-disable-line
          const tempnum = parseInt(element.Num); // eslint-disable-line
          // Twitch Clips Issues (some clips thumbnail medium url 404 not found)
          if ((tempnum <= 68) || (tempnum === 119) || (tempnum === 120)) {
            // tiny
            element.thumbnail_url = element.thumbnail_url.replace('480x272', '86x45'); // eslint-disable-line
          } else {
            // medium
            element.thumbnail_url = element.thumbnail_url.replace('480x272', '260x147'); // eslint-disable-line
          }
      ...
      if (this.usertempset.size > 0) {
        ...
        const UserResponse = await this.getTwitchRequest(TempURL);
        if (UserResponse) {
      ...
      /** Get Game Data * */
      const GamesBaseURL = 'games?';
      if (this.gametempset.size > 0) {
        TempURL = GamesBaseURL;
        this.gametempset.forEach((element) => {
          TempURL += `&id=${element}`;
        });
        const GamesResponse = await this.getTwitchRequest(TempURL);
        if (GamesResponse) {
    ...
    async getJSON() {
      ...
      this.clipList = new Array(this.clipJSON.length).fill().map(() => ({
        Num: 'XXX',
        id: '',
        url: '',
        title: 'ERROR',
